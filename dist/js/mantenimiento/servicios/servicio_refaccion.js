...
var api_url = url_root + '/api/servicios-refacciones/' // URL API servicio
...
var elementos = {
    modal: $('#myModal'),
    form_modal: $('#form-servicio-refaccion'),
    tabla: $('#example1'),
...
$(document).ready(function () {
    ...
    $(elementos.form_modal).on('submit', function (e) {
        ...
        data += accion == 'Nuevo' ? '&id_servicio=' + elementos.id_servicio : '&id_servicio=' + elementos.id_servicio + '&id=' + datos_temp.id
        $.ajax({
            url: api_url,
            data: data,
            method: metodo,
...
function llenarTabla() {
    elementos.id_servicio = elementos.tabla.data('id')
    elementos.datatable = $(elementos.tabla).DataTable({
        "autoWidth": false,
        responsive: true,
        "ajax": api_url + '?id_servicio=' + elementos.id_servicio,
        "columns": [
